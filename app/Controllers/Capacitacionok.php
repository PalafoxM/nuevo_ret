	...
	public function index()
	{
		if($this->session->get('logged'))
		{
			...
			$data['form_field']		=		[
												...
												['texto', 'Seleccione la modalidad', '', '', '', '', '', '', '', '', ''],
												['checkbox', 'Escuelas de licenciatura', 'lic', (($this->usuario_model->get($this->session->get('id'), 'lic', 'ret_frm_capacitacion', 'clave') == 1)?'checked':((isset($valueFlashdata['lic']))?'checked':'')), '', '', false, false, '', '', '','3'],
												['checkbox', 'Escuelas de bachillerato y técnicas', 'bac', (($this->usuario_model->get($this->session->get('id'), 'bac', 'ret_frm_capacitacion', 'clave') == 1)?'checked':((isset($valueFlashdata['bac']))?'checked':'')), '', '', false, false, '', '', '','3'],
												['checkbox', 'Centros de capacitación', 'cca', (($this->usuario_model->get($this->session->get('id'), 'cca', 'ret_frm_capacitacion', 'clave') == 1)?'checked':((isset($valueFlashdata['cca']))?'checked':'')), '', '', false, false, '', '', '','3'],
												['checkbox', 'Instructores', 'ins', (($this->usuario_model->get($this->session->get('id'), 'ins', 'ret_frm_capacitacion', 'clave') == 1)?'checked':((isset($valueFlashdata['ins']))?'checked':'')), '', '', false, false, '', '', '','3'],
												['hr'],
